...
public class PositioningHelper {
    ...
    public static int getActualX(AlignmentSizeOffset alignmentSizeOffset) {
        ...
        int result = 0;
        switch(alignmentSizeOffset.getAlignment().getHAlignment()) {
            case LEFT:
                result = alignmentSizeOffset.getOffsetX();
                break;
            case RIGHT:
                result = windowWidth - alignmentSizeOffset.getWidth() - alignmentSizeOffset.getOffsetX();
                break;
            case MIDDLE:
                result = (windowWidth - alignmentSizeOffset.getWidth()) / 2 + alignmentSizeOffset.getOffsetX();
        }

        switch(alignmentSizeOffset.getScreenAlignment().getHAlignment()) {
            case RIGHT:
                result = windowWidth - alignmentSizeOffset.getWidth() - result;
                break;
            case MIDDLE:
                result = ((windowWidth - alignmentSizeOffset.getWidth()) / 2) + result;
        }

        result = MathUtils.clamp(result, 0, windowWidth - alignmentSizeOffset.getWidth());
    ...
    public static int getActualY(AlignmentSizeOffset alignmentSizeOffset) {
        ...
        switch (alignmentSizeOffset.getAlignment().getVAlignment()) {
            case TOP:
                result = alignmentSizeOffset.getOffsetY();
                break;
            case BOTTOM:
                result = windowHeight - alignmentSizeOffset.getHeight() - alignmentSizeOffset.getOffsetY();
                break;
            case CENTER:
                result = (windowHeight - alignmentSizeOffset.getHeight()) / 2 + alignmentSizeOffset.getOffsetY();
        }

        switch(alignmentSizeOffset.getScreenAlignment().getVAlignment()) {
            case BOTTOM:
                result = windowHeight - alignmentSizeOffset.getHeight() - result;
                break;
            case CENTER:
                result = ((windowHeight - alignmentSizeOffset.getHeight()) / 2) + result;
        }

        result = MathUtils.clamp(result, 0, windowHeight - alignmentSizeOffset.getHeight());
